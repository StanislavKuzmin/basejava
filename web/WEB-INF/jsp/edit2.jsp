...
<%@ page import="com.urase.webapp.Config" %>
  ...
  <div class="scrollable-panel">
    <div class="form-wrapper">
      ...
      <c:forEach var="type" items="<%=ContactType.values()%>">
        <input class="field" type="text" name="${type.name()}" size=30 placeholder="${type.name()}"
      ...
      <c:forEach var="type" items="<%=SectionType.values()%>">
        ...
        <c:choose>
          ...
          <c:when test="${type=='QUALIFICATIONS' || type=='ACHIEVEMENT'}">
            <textarea class="field" name='${type}'><%=String.join("\n", ((ListSimpleSection) section).getItems())%></textarea>
          </c:when>
          <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
            <c:forEach var="org" items="<%=((OrganizationSection) section).getOrganizations()%>" varStatus="counter">
              ...
              <input class="field" type="text" placeholder="Название" name='${type}' size=100 value="${org.nameEmployer}">
              <input class="field" type="text" placeholder="Ссылка" name='${type}url' size=100 value="${org.linkEmployer}">

              <%--                            <button class="small-green-button">Добавить должность</button>--%>

              <c:forEach var="pos" items="${org.periods}">
                <jsp:useBean id="pos" type="com.urase.webapp.model.Period"/>
